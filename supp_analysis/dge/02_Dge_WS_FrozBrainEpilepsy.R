...
exp <- exp[,grep("Lega|Ep",names(exp))]
#exp$EpBB04<-NULL
filter=apply(exp, 1, function(x) (all(x[grep("Lega",names(x))] > 0) | all(x[grep("Ep",names(x))] > 0)))
exp <- exp[filter,]
...
openxlsx::write.xlsx(outputSV, file = "Dge_Results/Dge_WS_FrozBrainEpilepsy.xlsx", colNames = TRUE, borders = "columns",sheetName="WS_Ep",row.names=T)
...
write.table(adj.residualsSV, "Shiny/expAdj_WS_FrozBrainEpilepsy.txt",sep="\t",quote=F)

pd$Type <- ifelse(pd$Diagnosis == "Lega","Type1","Type2")
write.table(pd, "Shiny/WS_FrozBrainEpilepsy_pheno.txt",sep="\t",quote=F)
