...
files1=list.files(pattern="SME_cor_LR")
...
save(FisherMat,file="SME_CorrelatedGenes_EnrichMat.RData")
...
vec <- c("Delta","Theta","Alpha","Beta","Gamma","High.Gamma")
...
#p$variable <- factor(p$variable,levels=c("ENDOTHELIAL","ASTROCYTES","OPC","OLIGODENDROCYTES","MICROGLIA","EXCITATORY", "INHIBITORY"))

pdf("Modules_SME_CorrelatedGenes_Bubble.pdf",width=8,height=2)
ggscatter(p, 
  			x = "variable",
  			y = "Module",
   			size="OR",
